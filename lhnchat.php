...
class LHNChat extends WP_Widget {
	...
	public function widget($args, $instance) {
	    ...
	    if($account && is_numeric($account) && $button && is_numeric($button)){
		echo $before_widget;
		...
		<script type="text/javascript">
			if(lhnButtonCheck === undefined){
				...
				lhnLoadEvent = addLHNButton(lhnScriptSrc,'append', lhnJsHost, lhnAccountN);
			}else{
				var lhnButtonN2 = <?php echo $button ?>;
				lhnLoadEvent = addLHNButton(lhnButtonN2,'insert', lhnJsHost, lhnAccountN);
			}
